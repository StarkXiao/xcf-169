...
import { GEAR_SIZES } from './MainScene'
...
export class CustomMainScene extends Phaser.Scene {
  ...
  private customGears: GearConfig[] = []

  constructor() {
    super('CustomMainScene')
  }

  init(data: CustomSceneData): void {
    this.customGears = data.gears || []
  }
  ...
  private refreshAllGearVisuals(): void {
    this.gearGraphicsMap.forEach((graphics, id) => {
      ...
      if (!container) return
      const gearConfig = this.customGears.find((g) => g.id === id)
  ...
  private createRain() {
    ...
    rainGraphics.generateTexture('customRainTexture', 20, 40)
    rainGraphics.destroy()

    const config = WEATHER_RAIN_CONFIG[this.currentWeather.rain]
    this.rainParticles = this.add.particles(0, -10, 'customRainTexture', {
      x: { min: 0, max: width },
      y: -10,
  ...
  private createWind() {
    ...
    windGraphics.generateTexture('customWindTexture', 40, 15)
    ...
    this.windParticles = this.add.particles(0, 0, 'customWindTexture', {
      x: -20,
      y: { min: 0, max: height },
  ...
  private createGears() {
    const gears = this.customGears.length > 0 ? this.customGears : []
